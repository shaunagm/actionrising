...
class ActionListView(LoginRequiredMixin, generic.ListView):
    ...
    model = Action
    queryset = Action.objects.filter(status__in=["rea", "fin"]).filter(current_privacy__in=[PrivacyChoices.public, PrivacyChoices.sitewide])

class PublicActionListView(generic.ListView):
    template_name = "actions/actions.html"
    model = Action
    queryset = Action.objects.filter(status__in=["rea", "fin"]).filter(current_privacy=PrivacyChoices.public)
