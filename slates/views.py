...
class SlateListView(LoginRequiredMixin, generic.ListView):
    ...
    model = Slate
    queryset = Slate.objects.filter(status__in=["rea", "fin"]).filter(current_privacy__in=[PrivacyChoices.public, PrivacyChoices.sitewide])

class PublicSlateListView(generic.ListView):
    template_name = "slates/slates.html"
    model = Slate
    queryset = Slate.objects.filter(status__in=["rea", "fin"]).filter(current_privacy=PrivacyChoices.public)
